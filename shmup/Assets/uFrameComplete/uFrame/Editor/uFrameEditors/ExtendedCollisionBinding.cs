...
public static class ExtendedCollisionBinding
{
    ...
    public static IObservable<T> OnComponentCollision2DWith<T>(this GameObject t, CollisionEventType eventType) where T : MonoBehaviour
    {
        return t.OnCollision2DObservable(eventType).Where(p => p.gameObject.GetComponent<T>() != null).Select(p => p.gameObject.GetComponent<T>());
    }
